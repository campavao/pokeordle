...
function TeraRaidBattle() {
  ...
  const updateCurrentTypes = useCallback(
    async (type, index) => {
      const data = await fetch(`https://pokeapi.co/api/v2/type/${type}`).then(
        (res) => res.json()
      );
  ...
  const updateTypesFromPokemon = useCallback(async (types) => {
    const data = await Promise.all(
      types.map(
        async (type) =>
          await fetch(`https://pokeapi.co/api/v2/type/${type}`).then((res) =>
            res.json()
          )
      )
    );
  ...
  const pokemonOptions = useMemo(
    () =>
      Pokedex.filter((pokemon) =>
        filterSuggestions(pokemon, guesses, 9999)
      ).map((pokemon) => {
  ...
  const handlePokeType = (e) => {
    if (e.length) {
      const found = Pokedex.find((poke) => poke.name.english === e[0]);
      if (found) {
        setCurrentTypes([]);
  ...
  useEffect(() => {
    if (currentPokemon && !currentTypes.length) {
      const newTypes = currentPokemon.types.map((name) => name.toLowerCase());
        ...
        <div className="type-list">
          <div>
            <div className="type-name">
              Type 1:
              {currentTypes[0] && (
                <TypeList types={[currentTypes[0]]} useTypeColors justShow />
              )}
            </div>
            <Typeahead
              id="input"
          ...
          <div>
            <div className="type-name">
              Type 2:
              {currentTypes[1] && (
                <TypeList types={[currentTypes[1]]} useTypeColors justShow />
              )}
            </div>{' '}
            <Typeahead
              id="input"
          ...
          <div>
            <div className="type-name">
              Tera Type:
              {teraTypes[0] && (
                <TypeList types={[teraTypes[0]]} useTypeColors justShow />
              )}
            </div>{' '}
            <Typeahead
              id="input"
              ...
              ref={teraRef}
              selected={
                currentPokemon && teraTypes[0] ? [teraTypes[0].name] : undefined
              }
            />
          </div>
        </div>
