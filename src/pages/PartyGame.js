...
function PartyGame() {
    ...
    const handleClick = (e) => {
        ...
        const newRemainingTypes = solution
            .filter(
                (solutionPokemon) => !guessedIds.includes(solutionPokemon.id)
            )
            .map((solutionPokemon) => solutionPokemon.type.flat())
            .flat();
        setRemainingTypes(newRemainingTypes);
        if (
            guessBoard.every((guessPokemon) => guessPokemon.index.isSameIndex)
        ) {
            setHasWon(true);
        }
    };
    ...
    if (window.innerWidth < 1000) {
        return (
            <div>
                Sorry, not available for mobile yet. Please play this mode on
                Desktop.
            </div>
        );
    }
                                ...
                                return (
                                    <div className="party-slot">
                                        ...
                                        <Typeahead
                                            ...
                                            onChange={(poke) =>
                                                handleTypeAhead(poke, index)
                                            }
                                            placeholder={`#${index + 1}`}
                                            options={Array.from(pokedex)
                                                .filter((pokemon) =>
                                                    filterPartySuggestions(
                                                        pokemon,
                                                        guesses,
                                                        151
                                                    )
