...
export const getFilterFromGuess = (guess, pokemon) => {
  ...
  const includeMonoType =
    amountOfTypes && pokemon.types?.length === guess.types.length ? ['X'] : [];
  const excludeMonoType =
    amountOfTypes && pokemon.types?.length !== guess.types.length ? ['X'] : [];

  const correctEvolutionStage = guess.evolutionStage === pokemon.evolutionStage;
...
export const getBaseStats = (guessPokemon) => {
  const { hp, attack, defense, spAttack, spDefense, speed } = guessPokemon.base;
...
export function getFilters(guesses, pokemon) {
  const types = Array.from(
    new Set(guesses.map(({ types }) => types.map((type) => type).flat()).flat())
  );
  const guessedTypes = {
    ...types.reduce(
      (r, type) => ({
...
export const filterSuggestionsWithFilterState = (pokemon, filter) => {
  ...
  if (
    ...
    return hasBst && (!amountOfTypes || pokemon.types.length === amountOfTypes);
  }
};
